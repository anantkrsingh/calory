...
export function toPlan(row: PlanRow): Plan {
  return {
    ...
    revenueCatEntitlementIds: row.revenueCatEntitlementIds ?? [],
    chatMessagesLimit: orUndefined(row.chatMessagesLimit),
    tokensLimit: orUndefined(row.tokensLimit),
    repsHistoryDays: orUndefined(row.repsHistoryDays),
...
export function toNotificationDelivery(
  ...
  return {
    ...
    platform: orUndefined(row.platform),
    status: row.status,
